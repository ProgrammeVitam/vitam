...
if [ $# -ne 1 ]; then # Argument number must be one
  ...
  if [ ! -f "$1" ];then # if the file wich will be scan is existing, keep going
    ...
    scan -aubi "$1" 1> ${OUTPUT_DIR}/stdout 2> ${OUTPUT_DIR}/stderr # scanning the file and store the output
    RET=$? # return code of scan

    # Always output scan outputs to our own stderr
    (>&2 cat ${OUTPUT_DIR}/stdout ${OUTPUT_DIR}/stderr)

    if [ ${RET} -eq ${RET_VIRUS_FOUND_FIXED} ] ; then
      RET=2 # if virus found scan return 1; the script must return 2
      (>&1 cat ${OUTPUT_DIR}/stdout | grep `basename ${1}` | cut -d$'\t' -f 2-) # sending the list of virus to our own stdout
    elif [ ${RET} -eq 2 ] ; then
      RET=3 # if scan not performed, scan return 2; the script must return 3
