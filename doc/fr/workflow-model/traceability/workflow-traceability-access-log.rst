Processus de sauvegarde des journaux des accès
###############################################
...
Cette section décrit la sauvegarde des journaux des accès aux binaires mis en place dans la solution logicielle Vitam. Contrairement aux autres sécurisations de journaux de cycles de vie ou du journal des opérations, celle-ci n’est pas utilisée au sein d’un workflow.

Sauvegarde des journaux des accès (vision métier)
===================================================

Le processus de sauvegarde des journaux des *accès aux binaires* consiste en la création d’un fichier *.log* contenant : un certain nombre d’informations comme la date des accès, l'ID du document récupéré, le contrat utilisé, l'AU donnant accès au binaire, et l'ID de la requête d'accès.
...
* **Règle** : sauvegarde des journaux des accès
* **Type** : bloquant
* **Status** :
