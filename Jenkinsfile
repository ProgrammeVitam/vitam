                ...
                dir('sources') {
                    script {
                        docker.withRegistry('http://pic-prod-docker.vitam-env') {
                            docker.image('pic-prod-docker.vitam-env/elasticsearch/elasticsearch:6.6.0').withRun('-p 9200:9200 -p 9300:9300 -e "discovery.type=single-node" -e "cluster.name=elasticsearch-data"') { c ->
                            		docker.withRegistry('http://pic-prod-docker.vitam-env') {
