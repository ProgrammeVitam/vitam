                ...
                dir('sources') {
                    script {
                        docker.withRegistry('http://pic-prod-docker.vitam-env') {
                            docker.image('pic-prod-docker.vitam-env/elasticsearch:6.5.4').withRun('-p 9200:9200 -p 9300:9300 -e "ES_JAVA_OPTS=-Xms512m -Xmx512m" -m "1g" -e "discovery.type=single-node" -e "cluster.name=elasticsearch-data"') { c ->
