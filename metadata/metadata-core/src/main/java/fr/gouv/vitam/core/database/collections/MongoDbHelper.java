...
public class MongoDbHelper {
     ...
     * 
     * @param collection
     *            (not results except if already hashed)
     * @param ref
     * @return a VitamDocument<?> generic object from ID = ref value
     */
    public static final VitamDocument<?>
            findOneNoAfterLoad(final VitamCollections collection, final String ref) {
     ...
     */
    public static final VitamDocument<?> 
    		loadFromDocument(final VitamCollections coll, final Document obj)
     ...
     */
    public static final VitamDocument<?> 
    		findOne(final VitamCollections col, final String field, final String ref) {
    ...
    /**
     * Find the corresponding id in col collection if it exists. Calls
     * getAfterLoad
     *
     * @param col
     *            (not results except if already hashed)
     ...
     */
    public static final VitamDocument<?> 
    		findOne(final VitamCollections col, final String id) {
    ...
    public static final boolean exists(final VitamCollections col, final String id) {
        ...
        return col.getCollection().find(eq(VitamDocument.ID, id)).projection(MongoDbHelper.ID_PROJECTION)
                .first() != null;
    }
     ...
     * @param projection
     *            select condition
     * @return the FindIterable on the find request based on the given
     *         collection
     */
     ...
     * @return the UpdateResult on the update request based on the given
     *         collection
     */
     ...
     * @return the DeleteResult on the update request based on the given
     *         collection
     */
    ...
    /**
     * Add a Link according to relation defined, where the relation is defined
     * in obj1->obj2 way by default (even if symmetric)
     *
     * @param obj1
     * @param relation
     * @param obj2
     * @return a {@link BasicDBObject} that hold a possible update part (may be
     *         null)  as { $addToSet : { field : value } } or { field : value }
     */
