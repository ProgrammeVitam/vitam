...
public class OfferLogHelperTest {
    ...
    private void givenOfferLogOffsets(List<Integer> filesOffsets)
        ...
        doAnswer(
            args -> {
                Long startOffset = args.getArgumentAt(2, Long.class);
                int limit = args.getArgumentAt(3, Integer.class);

                return new RequestResponseOK<>()
                    .addAllResults(filesOffsets.stream()
                        .filter(o -> startOffset == null || o >= startOffset)
                        .limit(limit)
                        .map(o -> new OfferLog(o, LocalDateUtil.now(), "0_unit", "file" + o, "write"))
