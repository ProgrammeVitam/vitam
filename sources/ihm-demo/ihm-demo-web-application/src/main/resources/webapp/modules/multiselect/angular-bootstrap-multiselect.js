	...
	.directive("multiselect", ["$parse", "$document", "$compile", "$interpolate", "optionParser", function($parse, $document, $compile, $interpolate, optionParser) {
		return {
			...
			link    : function(originalScope, element, attrs, modelCtrl) {
				...
				scope.items = [];
				scope.header = "Informations supplémentaires";
				...
				function getHeaderText() {
					if(isEmpty(modelCtrl.$modelValue)) {
						scope.header = attrs.msHeader || "Informations supplémentaires";
						return scope.header;
					}
					...
					if(isMultiple) {
						if(attrs.msSelected) {
							...
							scope.header = modelCtrl.$modelValue.length + " sélectionnés";
						}
...
angular.module("multiselect.tpl.html", []).run(["$templateCache", function($templateCache) {
	$templateCache.put("multiselect.tpl.html",
		"<div class=\"btn-group\">\n" +
		"  <button tabindex=\"{{tabindex}}\" title=\"{{header}}\" type=\"button\" class=\"btn btn-default dropdown-toggle\" ng-click=\"toggleSelect()\" ng-disabled=\"disabled\" ng-class=\"{'error': !valid()}\" style=\"width: 230px\">\n" +
		"    <div style=\"padding-right: 13px; overflow: hidden; text-overflow: ellipsis;\">{{header}}</div><span class=\"caret\" style=\"position:absolute;right:10px;top:14px;\"></span>\n" +
		"  </button>\n" +
