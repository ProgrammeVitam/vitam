...
export class LogbookComponent extends PageComponent {
  ...
  public logbookData = [
    FieldDefinition.createIdField('obId', 'Identifiant de la demande d\'entrée', 6, 8),
  ...
  public columns = [
    ColumnDefinition.makeStaticColumn('obId', 'Identifiant de la demande d\'entrée',
      undefined, () => ({'width': '170px', 'overflow-wrap': 'break-word'}), false),
  ...
  (item) => (!!item.evDetData && JSON.parse(item.evDetData).EvDetailReq) ? JSON.parse(item.evDetData).EvDetailReq : '', undefined,
      () => ({'width': '195px', 'overflow-wrap': 'break-word'}), false),
    ColumnDefinition.makeSpecialValueColumn('Statut',
      (item) => item.events[1], LogbookComponent.handleStatus,
      () => ({'width': '120px'}), false),
    ColumnDefinition.makeSpecialValueColumn('Service transmetteur',
      (item) => (!!item.agIdExt && JSON.parse(item.agIdExt).TransferringAgency) ? JSON.parse(item.agIdExt).TransferringAgency : '', undefined,
      () => ({'width': '125px', 'overflow-wrap': 'break-word'}), false),
    ColumnDefinition.makeSpecialValueColumn('Service producteur',
      (item) => (!!item.agIdExt && JSON.parse(item.agIdExt).originatingAgency) ? JSON.parse(item.agIdExt).originatingAgency : '', undefined,
      () => ({'width': '125px', 'overflow-wrap': 'break-word'}), false),
    ...
    ColumnDefinition.makeIconColumn('Bordereau', ['fa-download'],
      ...
      () => ({'width': '90px'}), this.ingestUtilsService, false),
    ColumnDefinition.makeIconColumn('AR', ['fa-download'],
      LogbookComponent.downloadReports, LogbookComponent.displayReportDownload,
      () => ({'width': '40px'}), this.ingestUtilsService, false)
  ];
  public extraColumns = [
    ColumnDefinition.makeStaticColumn('evIdProc', 'Identifiant de l\'entrée', undefined,
      () => ({'width': '325px'}), false),
    ColumnDefinition.makeSpecialValueColumn('Service d\'archives',
      (item) => (!!item.agIdExt && JSON.parse(item.agIdExt).ArchivalAgency) ? JSON.parse(item.agIdExt).ArchivalAgency : '', undefined,
      () => ({'width': '125px', 'overflow-wrap': 'break-word'}), false),
    ColumnDefinition.makeSpecialValueColumn('Service versant',
      (item) => (!!item.agIdExt && JSON.parse(item.agIdExt).submissionAgency) ? JSON.parse(item.agIdExt).submissionAgency : '', undefined,
      () => ({'width': '125px', 'overflow-wrap': 'break-word'}), false),            
