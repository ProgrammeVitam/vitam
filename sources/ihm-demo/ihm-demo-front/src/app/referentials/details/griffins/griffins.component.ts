...
export class GriffinsComponent extends PageComponent {
  ...
  updateBreadcrumb(type: string) {
    if (type === 'accessionRegister') {
      ...
      this.newBreadcrumb = [
        ...
        {label: 'Services agents', routerLink: 'admin/search/griffins'},
  ...
  getDetail() {
    this.searchReferentialsService.getGriffinById(this.id).subscribe(
      (value) => {
        this.griffin = plainToClass(Griffin, value.$results)[0];
        this.searchReferentialsService.getFundRegisterById(this.griffin.Identifier, 1).subscribe((value) => {
